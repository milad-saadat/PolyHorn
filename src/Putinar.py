...
class Putinar:
    ...
    def get_lower_triangular_matrix(self, dim: int) -> ([[Polynomial]], [CoefficientConstraint]):
    ...
    def get_poly_sum(self, max_d:int) -> (Polynomial, [CoefficientConstraint]):
        ...
        if self.RHS.is_strict():
            all_constraints.append(CoefficientConstraint(strict_poly.monomials[0].coefficient, '>'))
    ...
    def handel_strict(self, max_d:int, power_of_new_var:int) -> [[CoefficientConstraint]]:
        ...
        for left_constraint in self.LHS:
            new_set_of_cons.append(PolynomialConstraint(left_constraint.polynomial.add_variables(new_variables)
                                                        , left_constraint.sign
                                                        )
                                   )
